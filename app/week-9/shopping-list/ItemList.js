  ...
  return (
    <div className="m-4">
      <div className="flex gap-2 mb-4 mx-2">
        <button
          className={`flex-1 px-4 py-2 rounded-lg font-bold ${sortBy === "name"
        ...
        <button
          className={`flex-1 px-4 py-2 rounded-lg font-bold ${sortBy === "category"
              ? "bg-green-500 text-white"
              : "bg-white text-black"
            }`}
      ...
      <ul className="grid grid-cols-2 gap-2 mx-2 mb-2">
        {sortedItems.map((item) => (
          <Item
            key={item.id} {...item} onSelect={() => onItemSelect(item)}
          />
        ))}
      </ul>
    </div>
